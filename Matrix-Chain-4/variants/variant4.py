...
def write_to_eventlog(csv_writer,run_id,timestamps,dims,num_threads):
    ...
    event0 = [id, "matmul(C,D)", utils.convert_timestamp_todtime(timestamps[0].numpy()), timestamps[1].numpy(), dims, num_threads]
    csv_writer.writerow(event0)

    event1 = [id, "matmul(B,T_CD)", utils.convert_timestamp_todtime(timestamps[1].numpy()),  utils.convert_timestamp_todtime(timestamps[2].numpy()), dims, num_threads]
