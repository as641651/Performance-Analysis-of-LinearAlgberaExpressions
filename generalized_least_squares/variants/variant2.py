...
def run(X, M, y):
    ...
    with tf.control_dependencies([tmp21]):
        stime5 = tf.timestamp()
    ...
    timestamps = [stime0, stime1, stime2, stime3, stime4, stime5, stime6, stime7, stime8, stime9]
...
def write_to_eventlog(csv_writer, exp_start_time, run_id, timestamps, dims, num_threads):
    ...
    event1 = [id, "tmp68 = (L7^-1 y)", timestamps[1].numpy(), timestamps[2].numpy(), dims, num_threads]
    ...
    event4 = [id, "tmp21 = (tmp12^T tmp68)", timestamps[4].numpy(), timestamps[5].numpy(), dims, num_threads]
    csv_writer.writerow(event4)

    event5 = [id, "(L15 L15^T) = tmp14", timestamps[5].numpy(), timestamps[6].numpy(), dims, num_threads]
    csv_writer.writerow(event5)

    event6 = [id, "tmp23 = (L15^-1 tmp21)", timestamps[6].numpy(), timestamps[7].numpy(), dims, num_threads]
    csv_writer.writerow(event6)

    event7 = [id, "tmp24 = (L15^-T tmp23)", timestamps[7].numpy(), timestamps[8].numpy(), dims, num_threads]
    csv_writer.writerow(event7)

    event8 = [id, "b = tmp24", timestamps[8].numpy(), timestamps[9].numpy(), dims, num_threads]
